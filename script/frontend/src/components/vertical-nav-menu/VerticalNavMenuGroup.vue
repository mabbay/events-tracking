    ...
    @mouseout  = "mouseout">
      ...
      <div @click="clickGroup" class="group-header w-full">
        <span class="flex items-center w-full">
          ...
          <unicon  :name="group.icon" :fill="'#fff' "  icon-style="line"  />

          <!-- Group Name -->
          <span v-show="!verticalNavMenuItemsMin" class="truncate mr-3 select-none">{{ $t(group.name) }}</span>
        ...
        <!-- Group Tooltip -->
        <span class="vs-sidebar--tooltip">{{ $t(group.name) }}</span>
      </div>
      <!-- /Group Label -->

      <!-- Group Items -->
      <ul ref="items" :style="styleItems" class="vs-sidebar-group-items rounded-lg">
        <li v-for="(groupItem, index) in group.submenu" :key="index">
          ...
          <v-nav-menu-item
            ...
            :target = "groupItem.target">
              <span class="truncate">{{ $t(groupItem.name) }}</span>
...
export default {
  ...
  computed: {
    ...
    styleItems() {
      return { maxHeight: this.maxHeight ,background: 'rgb(38, 44, 73)'}
    },
    itemIcon() {
      return (index) => {
  ...
  watch: {
    ...
    '$route'() {
      ...
      let scrollHeight = typeof(this.scrollHeight) != "undefined" ? this.scrollHeight : null;
