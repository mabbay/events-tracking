<template>
  <div class="Form">
    <form @submit.prevent="submit" method="post" :id="config.ref" v-if="ready">
      ...
      <div class="flex flex-no-wrap bg-light border-0 border-t border-b border-solid border-grey-light p-2 justify-end" >
          <button class="text-primary border border-primary border-solid bg-white py-2 px-10 rounded-lg cursor-pointer text-lg btn_shadow" >
            <span>{{ 'apply' | t }}</span>
          </button>
          <div class="w-2" ></div>
          <vs-button class="text-secondary border border-grey border-solid bg-white py-2" color="grey" @click="onClick('reset')">
              <span>{{ 'reset' | t }}</span>
          </vs-button>
      </div>
    </form>
    ...
    methods: {
      ...
      reset() {
        for (const key in this.formElements) {
          ...
          if (this.formElements[key].type == "start-end-date")
            this.formElements[key].value = { from: null, to: null };
        }
        this.refresh();
      },
      submit() {
...
.btn_shadow:hover{
    box-shadow: 0 9px 28px -9px rgba(var(--vs-primary), 1);
}
</style>