  ...
  <div id="verticalNavMenuItem"
    ...
    :class="[
      ...
      <router-link
        ...
        :to="{ path: to, name: name}"
        :target="target" >
          <!-- <vs-icon v-if="!featherIcon" :icon-pack="iconPack" :icon="icon" />
          <feather-icon v-else :class="{'w-3 h-3': iconSmall}" :icon="icon" /> -->
          <unicon :height="iconSmall ? 12 : 21" :width="iconSmall ? 12 : 21" :name="icon" :fill="activeLink ? '#fff'  : '#fff'" icon-style="line"  />
          <slot />
      </router-link>
