module.exports = class Controller {
    ...
    async createMany({ body }) { //! to be tested (body validation)
        const { error } = new this.Validation(body);
        if (error) return this.send(error.details[0].message, 400);
        const result = await this.Repository.bulkCreate(body);
        if (result.errors) return this.send(result.message, 400);
        return this.send(result);
    }
    async deleteMany({ filters }) {
        const result = await this.Repository.bulkRemove(filters);
        if (result.errors) return this.send(result.message, 400);
        return this.send(result);
    }
}
